...
e2 <- ggAcf(egarch.fit.stdres) + 
  labs(title = paste0(ser_name,': EGARCH residuals')) +
  theme_bw()
printer(e2, folder, subfolder, paste0(name,'_egarch_fit_acf'))

e3 <- ggAcf(egarch.fit.stdres^2) + 
  labs(title = paste0(ser_name,': EGARCH squared residuals')) +
  theme_bw()
...
e4 <- ggplot(data = fortify(egarch.fit.stdres), aes(sample = egarch.fit.stdres)) +
  ...
  labs(title = paste0(ser_name,': QQ-Plot of the EGARCH residuals'), y = 'Sample', x = 'Theoretical') +
  theme_bw()
...
oos.all$EGARCH <- egarch.result$sigma
