...
sinker(xtable(summary(ret)), name, name = paste0(name,'_ret_summary'))
sinker(xtable(table.Stats(ret), digits = 4), name, name = paste0(name,'_ret_stats'))
...
sinker(xtable(summary(is)), name, name = paste0(name,'_is_summary'))
sinker(table.Stats(is), name, name = paste0(name,'_is_stats'))
...
sinker(xtable(summary(oos)), name, name=paste0(name,'_oos_summary'))
sinker(table.Stats(oos), name, name = paste0(name,'_oos_stats'))
...
isos1 <- ggplot(data = fortify(is), aes(x = Index, y = is)) +
  ...
  ylab('log-returns') +
  theme_bw()
printer(isos1, name, name = paste0(name,'_ret_is'))
...
isos2 <- ggplot(data = fortify(oos), aes(x = Index, y = oos)) +
  ...
  ylab('log-returns') +
  theme_bw()
printer(isos2, name, name = paste0(name,'_ret_oos'))
...
isos3 <- ggplot() +
  ...
  theme_bw()
printer(isos3, name, name = paste0(name,'_isos_split'))
...
isos4 <- ggplot() +
  ...
  theme_bw()
printer(isos4, name, name = paste0(name,'_ret_abs'))

rm(isos1,isos2,isos3,isos4)
